...
func FillSliceCtrlMsg(ctrlType string, ctrlMsg Request) xapp.Slice_ctrl_msg_t {
	...
	if ctrlType == "ADDMOD" {
		...
		slices := xapp.NewSlice_array(int64(ctrlMsg.NumSlices))
		//slices = xapp.Slice_array(ctrlMsg.(map[string]interface{})["slices"].(xapp.Slice_array))

		for i := 0; i < ctrlMsg.NumSlices; i++ {
			slices.Setitem(int64(i), CreateSlice(ctrlMsg.Slices[i], ctrlMsg.SliceSchedAlgo))
		}
		...
		del_dl_id := xapp.NewDel_dl_array(int64(ctrlMsg.NumDlSlices))

		for i := 0; i < ctrlMsg.NumDlSlices; i++ {
			del_dl_id.Setitem(int64(i), uint(ctrlMsg.DeleteDlSliceId[i]))
		}
		...
		assoc := xapp.NewUe_slice_assoc_array(int64(ctrlMsg.NumUes))
