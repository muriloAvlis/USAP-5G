...
from ..asn1coder.e2sm_kpm_packer import e2sm_kpm_packer
from .e2sm_module import e2sm_types
...
def ntp_ts_to_unix_ms(ntp_timestamp) -> int:
    # Offset between NTP and Unix epochs (1900-1970 in seconds)
    ntp_epoch_offset = 2208988800

    # Extract seconds part (first 32 bits)
    unix_timestamp = (ntp_timestamp >> 32) - ntp_epoch_offset  # in seconds

    # Extract fractional part (last 32 bits) and convert to milliseconds
    fractional_seconds = (ntp_timestamp & 0xFFFFFFFF) / \
        (1 << 32)
    fractional_ms = fractional_seconds * 1000

    # Unix timestamp in ms
    unix_ms = (unix_timestamp * 1000) + fractional_ms
...
class e2sm_kpm_module(e2sm_kpm_packer):
    ...
    def subscribe_report_service_style_1(self, e2_node_id, reportingPeriod, metric_names, granulPeriod, indication_callback):
        ...
        '''
        event_trigger_def = self.encode_event_trigger_def(
            reportingPeriod)
        action_def = self.encode_action_def_format1(
            metric_names, granulPeriod)
        self.parent.subscribe(e2_node_id, self.ran_func_id, event_trigger_def,
                              action_def, indication_callback, e2sm_types.E2SM_KPM)
    ...
    def subscribe_report_service_style_2(self, e2_node_id, reportingPeriod, ue_id, metric_names, granulPeriod, indication_callback):
        ...
        event_trigger_def = self.encode_event_trigger_def(
            reportingPeriod)
        action_def = self.encode_action_def_format2(
            ue_id, metric_names, granulPeriod)
        self.parent.subscribe(e2_node_id, self.ran_func_id, event_trigger_def,
                              action_def, indication_callback, e2sm_types.E2SM_KPM)
    ...
    def subscribe_report_service_style_3(self, e2_node_id, reportingPeriod, matchingConds, metric_names, granulPeriod, indication_callback):
        ...
        event_trigger_def = self.encode_event_trigger_def(
            reportingPeriod)
        action_def = self.encode_action_def_format3(
            matchingConds, metric_names, granulPeriod)
        self.parent.subscribe(e2_node_id, self.ran_func_id, event_trigger_def,
                              action_def, indication_callback, e2sm_types.E2SM_KPM)
    ...
    def subscribe_report_service_style_4(self, e2_node_id, reportingPeriod, matchingUeConds, metric_names, granulPeriod, indication_callback):
        ...
        event_trigger_def = self.encode_event_trigger_def(
            reportingPeriod)
        action_def = self.encode_action_def_format4(
            matchingUeConds, metric_names, granulPeriod)
        self.parent.subscribe(e2_node_id, self.ran_func_id, event_trigger_def,
                              action_def, indication_callback, e2sm_types.E2SM_KPM)
    ...
    def subscribe_report_service_style_5(self, e2_node_id, reportingPeriod, ue_ids, metric_names, granulPeriod, indication_callback):
        ...
        event_trigger_def = self.encode_event_trigger_def(
            reportingPeriod)
        action_def = self.encode_action_def_format5(
            ue_ids, metric_names, granulPeriod)
        self.parent.subscribe(e2_node_id, self.ran_func_id, event_trigger_def,
                              action_def, indication_callback, e2sm_types.E2SM_KPM)

    def decode_ric_indication(self, ric_indication):
        indication_hdr = self.decode_indication_header(
            ric_indication.indication_header)
        indication_msg = self.decode_indication_message(
            ric_indication.indication_message)
    ...
    def _extract_meas_data_ind_msg_f1(self, indication_msg_content):
        ...
        for measDataItem in measData:
            ...
            for measRecordItem in measRecord:
                valueType = measRecordItem[0]  # doesn't used
    ...
    def _extract_content_ind_msg_f1(self, indication_msg):
        ...
        {'indicationMessage-formats': ('indicationMessage-Format1', {
            'measData': [{'measRecord': [('integer', 8), ('float', 8.5)]}],
    ...
    def extract_meas_data(self, indication_msg):
        '''
        Check indication message format
        '''
        meas_data = {}
