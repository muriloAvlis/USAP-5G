...
void testEncodeEventTriggerDefinitionFormat1(u_int64_t reportingPeriod)
{
    encodedData_t res = encodeEventTriggerDefinitionFormat1(reportingPeriod);
    printf("Bytes encoded: %d\n", res.size);
    printf("Bytes list: [");
    for (size_t i = 0; i < res.size; i++)
    {
        printf(" %hhu ", res.buffer[i]);
    }
...
void testEncodeActionDefinitionFormat4(char **metricNames, const size_t numOfMetrics, const u_int64_t granularityPeriod)
{
    encodedData_t res = encodeActionDefinitionFormat4(metricNames, numOfMetrics, granularityPeriod);
    printf("Bytes encoded: %d\n", res.size);
    printf("Bytes list: [");
    for (size_t i = 0; i < res.size; i++)
    {
...
int main()
{
    // test Event Trigger Definition Format 1
    testEncodeEventTriggerDefinitionFormat1(10000);

    // test Action Definition Format 4
    char * metricNames[] = {"CQI", "RSRP", "RSRQ"};
    const size_t numberOfMetrics = 3;
    const u_int64_t granPeriod = 1000;

    testEncodeActionDefinitionFormat4(metricNames, numberOfMetrics, granPeriod);
