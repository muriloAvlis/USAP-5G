...
void Kpm_monitor::kpm_sm_cb(sm_ag_if_rd_t const* rd)
{
...

    {
        ...
        SPDLOG_DEBUG("Receiving KPM indication message number {:d} with latency {:.2f} ms", counter, latency);
...
// Get UEs by S-NSSAI criteria (PS: SD not implemented)
test_info_lst_t Kpm_monitor::gen_snssai_test_info_lst(u_int8_t sst)
{
    ...
    *test_info_lst.test_cond = {EQUAL_TEST_COND};
    ...
    test_info_lst.test_cond_value->octet_string_value->buf = static_cast<u_int8_t*>(calloc(
        test_info_lst.test_cond_value->octet_string_value->len, sizeof(u_int8_t)));
    assert(test_info_lst.test_cond_value->octet_string_value->buf != nullptr);

    test_info_lst.test_cond_value->octet_string_value->buf[0] = {sst};
...
test_info_lst_t Kpm_monitor::gen_ul_rsrp_test_info_lst(int64_t testValue)
{
    ...
    // fill test condition type
    test_info_lst.test_cond_type = {UL_RSRP_TEST_COND_TYPE};
    // It can only be TRUE_TEST_COND_TYPE so it does not matter the type
    // but ugly ugly...
    test_info_lst.UL_RSRP = {TRUE_TEST_COND_TYPE};

    // fill test condition
    test_info_lst.test_cond = static_cast<test_cond_e*>(calloc(1, sizeof(test_cond_e)));
    assert(test_info_lst.test_cond != nullptr);
    *test_info_lst.test_cond = {GREATERTHAN_TEST_COND};

    // fill test value
    test_info_lst.test_cond_value = static_cast<test_cond_value_t*>(calloc(1, sizeof(test_cond_value_t)));
    assert(test_info_lst.test_cond_value != nullptr);
    test_info_lst.test_cond_value->type = INTEGER_TEST_COND_VALUE;
    test_info_lst.test_cond_value->int_value = static_cast<int64_t*>(calloc(1, sizeof(int64_t)));
    assert(test_info_lst.test_cond_value->int_value != nullptr);
    *test_info_lst.test_cond_value->int_value = testValue;
    ...
    case STYLE_4_RIC_SERVICE_REPORT:
        {
            ...
            constexpr size_t len_slices = 1;
            // 1 => eMBB | 2 => URLLC | 3 => mIoT | 128 => default
            std::array<u_int8_t, len_slices> slices{128};

            act_def.frm_4.matching_cond_lst_len = len_slices;
            ...
            // PS: OAI-RAN not support OR CONDITION, so only a slice is supported :(
            for (size_t i = 0; i < slices.size(); i++)
            {
                act_def.frm_4.matching_cond_lst[i].test_info_lst = gen_snssai_test_info_lst(slices.at(i));
            }
...
Kpm_monitor::Kpm_monitor()
{
    ...
    if (report_period && std::strlen(report_period) > 0)
    {
        ...
        {
            SPDLOG_ERROR(
                "Invalid argument: '{}' cannot be converted to an integer, setting REPORT_PERIOD to default value",
                report_period);
            REPORT_PERIOD = 1000; // default value
        } catch (const std::out_of_range& e)
        {
            SPDLOG_ERROR(
                "Out of range: '{}' is too large to be converted to an integer, setting REPORT_PERIOD to default value",
                report_period);
            REPORT_PERIOD = 1000; // default value
        }
    }
...
void Kpm_monitor::Start()
{
    ...
    for (size_t i = 0; i < nodes.len; i++)
    {
        ...
        if (kpm_idx == -1)
        {
            SPDLOG_WARN("KPM SM ID could not be found in the RAN Function List of E2 node {}, ignoring...",
                        node->id.nb_id.nb_id);
            continue;
        }
        ...
        {
            SPDLOG_INFO("Node {} with ID {} support KPM Report Style, sending subscription...",
                        get_ngran_name(node->id.type), node->id.nb_id.nb_id);
